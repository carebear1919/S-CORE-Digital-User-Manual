...
export default function ManualImage({ caption }: ManualImageProps) {
  ...
  return (
    <div className="my-6 no-print" id={`screenshot-container-${slug}`}>
      ...
      <div className="w-full border border-gray-200 rounded-2xl overflow-hidden bg-white shadow-md">
        ...
        <div className="bg-gray-50 border-b border-gray-200 px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-[#ff5f56]/80 inline-block"></span>
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-[#ffbd2e]/80 inline-block"></span>
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-[#27c93f]/80 inline-block"></span>
          </div>
          <div className="bg-white border border-gray-200/80 rounded-lg text-[8px] sm:text-[10px] text-gray-500 font-mono px-2 sm:px-3 py-1 w-2/3 md:w-1/2 text-center truncate select-none shadow-xs">
            https://s-core-system.dlsu-d.edu.ph/{slug}
          </div>
          <div className="w-8 sm:w-12"></div> {/* Spacer balance */}
        </div>
        ...
        <div className="bg-gray-50 p-1 md:p-2 max-h-[550px] overflow-y-auto flex flex-col items-center justify-start scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          {!imgError ? (
            ...
            <div className="w-full py-10 sm:py-16 px-4 sm:px-6 bg-white rounded-lg border border-dashed border-gray-300 flex flex-col items-center justify-center text-center space-y-4">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-emerald-50 text-emerald-800 flex items-center justify-center shadow-xs">
                <Camera size={22} />
              </div>
              
              <div className="space-y-1">
                <h4 className="text-xs sm:text-sm font-semibold text-gray-800 font-sans uppercase tracking-wider">
                  [ {caption} ]
                </h4>
                <p className="text-xs text-gray-500 max-w-md font-sans leading-relaxed px-2">
              ...
              <div className="bg-emerald-50/50 border border-emerald-800/10 rounded-xl px-3 sm:px-4 py-2.5 max-w-md text-left font-sans">
      ...
      <p className="text-xs text-center text-gray-500 mt-2 font-medium italic px-2 sm:px-0">
